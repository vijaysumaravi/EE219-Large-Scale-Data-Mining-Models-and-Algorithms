...
total = fetch_20newsgroups(subset = 'all',  shuffle=True, random_state=42);
...
# totalNMF += 0.00001  
# logData = np.log(totalNMF)
# logData[np.where(~np.isfinite(logData))] = totalNMF[np.where(~np.isfinite(logData))]

kmeans = KMeans(n_clusters=20, n_init=30).fit(normData)
...
# x1 = normData[kmeans.labels_ == 0][:, 0]
# y1 = normData[kmeans.labels_ == 0][:, 1]

# plt.figure()
# plt.plot(x1,y1,'g+')
# x2 = normData[kmeans.labels_ == 1][:, 0]
# y2 = normData[kmeans.labels_ == 1][:, 1]

# plt.plot(x2, y2, 'r+')
# plt.title("2D Projection of Feature Vectors ")


# plt.savefig("clustersNorm2dNMF.png", format='png')
