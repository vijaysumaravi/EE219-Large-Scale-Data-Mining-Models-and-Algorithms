...
ovo_svc = OneVsRestClassifier(svm.SVC(kernel='linear', probability=True, C = 1000))


ovo_svc.fit(train_nmf, train_target_group)
predicted = ovo_svc.predict(test_nmf)
print_statistics(test_target_group, predicted)

predicted_probs = ovo_svc.decision_function(test_nmf)
