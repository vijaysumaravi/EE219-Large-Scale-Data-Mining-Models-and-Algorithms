...
def plot_roc(fpr, tpr):
	...
	plt.title("ROC curve for SVM Classifier using 5 fold Cross Validation")
	ax.set_xlim([-0.1, 1.1])
...
def findBestParameter(train, train_nmf, test, test_nmf):
...
best_param = findBestParameter(twenty_train, train_nmf, twenty_test, test_nmf)
print "Best Score was obtained for k = ", best_param



classifier = svm.SVC(kernel='linear', probability=True, C = 10 ** -(best_param))
classifier.fit(train_nmf, train_target_group)
