...
regularizationCoeffecients = [0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000]

def plot_roc(fpr, tpr, penalty):
    ...
    plt.title("ROC curve for Logistic Regression withregualarization coeffecient = " + str(penalty) )
    ax.set_xlim([-0.1, 1.1])
    ...
    plt.savefig("LRroc" + str(penalty)+ ".png")

def print_statistics(actual, predicted, penalty):
    print "Regularization parameter is :", penalty , "\n"
...
def plot_confusion_matrix(cm, penalty, classes,
                          normalize=False,
    ...
    plt.savefig("LR"+str(normalize) +str(penalty)+  ".png")
...
for coeff in regularizationCoeffecients:

    lr_norm2 = linear_model.LogisticRegression(penalty = 'l2' , C=coeff)
    lr_norm2.fit(train_nmf, train_target_group)

    lr_predicted = lr_norm2.predict(test_nmf)
    lrPredictedScore = lr_norm2.decision_function(test_nmf)
    lrPredictedProbs = lr_norm2.predict_proba(test_nmf)

    print_statistics(test_target_group, lr_predicted, coeff)
    fpr, tpr, _ = roc_curve(test_target_group, lrPredictedProbs[:,1])
    plot_roc(fpr, tpr, coeff)
    cnf_matrix = smet.confusion_matrix(test_target_group, lr_predicted)
    plot_confusion_matrix(cnf_matrix, penalty=coeff, classes=class_names, title='Confusion matrix without normalization')
    plot_confusion_matrix(cnf_matrix, penalty=coeff, classes=class_names, normalize=True, title='Confusion matrix with normalization' )
