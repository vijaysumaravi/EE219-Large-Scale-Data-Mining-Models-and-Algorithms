...
def plot_roc(fpr, tpr, C):
	...
	plt.savefig("svmNMFRoc" + str(C) + ".png")

def print_statistics(actual, predicted, penalty):
	print "Gamma is", penalty , "\n"
...
def plot_confusion_matrix(cm, classes, penalty,
                          normalize=False,
    ...
    plt.savefig("cf"+str(normalize) + str(penalty) + ".png")
...
for penalty in penalties:
	...
	classifier = svm.SVC(kernel='linear', probability=True, C = penalty)
	classifier.fit(train_nmf, train_target_group)
	predicted = classifier.predict(test_nmf)
	print_statistics(test_target_group, predicted, penalty)
	predicted_probs = classifier.predict_proba(test_nmf)
	fpr, tpr, _ = roc_curve(test_target_group, predicted_probs[:,1])
	plot_roc(fpr, tpr, penalty)
	cnf_matrix = smet.confusion_matrix(test_target_group, predicted)
	plot_confusion_matrix(cnf_matrix,penalty=penalty, classes=class_names, title='Confusion matrix without normalization')
	plot_confusion_matrix(cnf_matrix, penalty=penalty, classes=class_names, normalize=True, title='Confusion matrix with normalization' )
